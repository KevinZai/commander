...
- `commander/cowork-plugin/` — Desktop plugin (50 skills, 17 sub-agent personas, 8 hooks × 16 handlers, 2 credential-free bundled MCP servers per `.mcp.json` + 16 opt-in via `/ccc-connect`)
...
│   ├── skills/                  # 50 plugin skills (23 ccc-* workflows + 5 lifecycle + 2 vendor-sourced + 20 other)
...
**Stats:** 502 skills total, 50 plugin skills, 17 sub-agent personas, 8 lifecycle hooks (16 handlers), 2 bundled MCP servers + 16 opt-in, 83 commands, 28 JS hooks, 19 vendors, 10 themes
...
CCC ships 50 focused `/ccc-*` plugin skills as the curated front door, then routes into 450+ CCC ecosystem skills across 11 domains. It is a meta-layer over the broader Claude skill ecosystem — not a replacement for it.
...
- Everything free forever: 50 plugin skills, 17 agents, 2 credential-free bundled MCP servers (context7 + sequential-thinking) + 16 opt-in via /ccc-connect, 8 lifecycle hooks × 16 handlers. No feature gating, no paywalls, no license checks.
