...
CC Commander's primary product as of v4.1.0-beta.2. Claude Cowork Desktop / Claude Code Desktop install:
...
### Architecture note (v4.1.0-beta.2+)
...
**Stats:** 502 skills total, 60 plugin skills, 22 sub-agent personas, 23 lifecycle hooks (38 handlers), 2 bundled MCP servers + 16 opt-in, 83 commands, 28 JS hooks, 19 vendors, 10 themes
...
- 60 `/ccc-*` plugin skills are the curated front door; they route into 502+ deeper skills
- `CONNECTORS.md` defines 13 tool-agnostic connector categories via `~~category` placeholders — skills are tool-agnostic
- Everything free for now: 60 plugin skills, 22 agents, 2 credential-free bundled MCP servers (context7 + sequential-thinking) + 16 opt-in via /ccc-connect, 23 lifecycle hooks × 38 handlers. No feature gating, no paywalls, no license checks.
