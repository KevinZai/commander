...
**Current Release:** v7.4.2 — full-audit release. An 11-agent adversarially-verified sweep of every subsystem, then everything confirmed got fixed: the PostCompact hook no longer emits output the harness rejects on every compaction (`emit.mjs` now guards the real 20-event `hookSpecificOutput` union, and the contract test drives every emitter under its real registered events); Mission Control's roster join now keys on `agent_id` so finished subagents actually show finished (was 78 of 3,337 matched); staleness warnings can actually fire (freshness stamps skip gap-filled all-zero telemetry rows); the Memory tab's "Shown" tile tells the truth; every shipped surface now tells the keyless story (MCP.md, Cursor/Windsurf adapters, and their templates dropped `COMMANDER_LICENSE_KEY`); `/ccc-connect` captures credentials via hidden-input terminal command instead of chat; `/ccc-infra` is scope-clean; `/ccc-tuneup` is clickable from the hub and all 11 domain routers picker on bare invocation; rtk vendor picked up six upstream security fixes alongside 7 routine vendor bumps; root npm audit is 0; and the doc-sync gate stopped comparing the contract against a machine-dependent skill count. Added in v7.4.1: `/ccc-update` and `/ccc-doctor` no longer dead-end on **Desktop-managed (account-synced) installs** — the mode with no marketplace clone and no `installed_plugins.json` entry *by design*. Both now detect three install modes (CLI marketplace / Desktop-managed / dev checkout), read the Desktop copy's real version, fetch latest from GitHub directly, and give mode-correct guidance. Added in v7.4.0: **One Commander.** The four published decks (`/ccc-browse`, `/ccc-mission-control`, `/ccc-usage`, `/ccc-safety`) are now tabs of a single **inline console**, `/ccc-console` (82nd plugin skill): Overview · Usage · Safety · **Memory** · **History** · Launch, rendered through the visualize MCP with a prompt bar that reaches the live session via `sendPrompt(text)`. The security model is the feature — chips are fixed template commands that display exactly what they will send, free text is newline-stripped and capped at 500 chars, and **no telemetry-, memory- or log-derived text ever reaches a `sendPrompt` payload** (anything that can append to a JSONL could otherwise compose a command the user appears to have typed). The four deck skills survive as "publish just this tab" exports on **their existing file paths**, so every bookmarked artifact URL keeps updating in place; `/ccc-console publish` absorbs the Cockpit's URL rather than minting a fifth. **Memory** reads your own claude-mem store read-only, titles only — claude-mem is AGPL and deliberately not bundled, so "not installed" is the normal zero-state, never an error. **History** is 30 days of telemetry Commander already writes (`metrics.jsonl` as the durable backbone, detail counted by filename) — no new collector, and per-message content is neither collected nor claimed. The console **opens itself once at session start** (local-only render; publishing stays consent-gated), off via `/ccc-console off`, `{"autoOpen": false}` in `~/.claude/commander/console.json`, or `CCC_NO_AUTOCONSOLE=1`. Under it all: one renderer and one composed model — the three deck libraries shed a net 1,009 lines of duplicated markup, proven byte-identical against pre-refactor goldens. **Fixed:** a read/write race that let identical telemetry produce different Usage numbers run to run (Mission Control persists `metrics.jsonl`; Usage reads it — the composed model now sequences writer before reader), and hook `timeout` values that were milliseconds in a seconds field, so the anti-hang ceilings had never been in force. `plugin.json` now sets `defaultEnabled: true`. Added in v7.3.2: **Anthropic + Codex CLI only.** Commander no longer routes work to Gemini or any local-inference runtime: the Gemini adapter is gone, executor preference is `claude` → `codex`, and `ccc-research/gemini-fallback` is rewritten as `large-context` (use Opus 5's native 1M window → reduce → chunk-and-synthesize → hand mechanical execution to the Codex CLI). The old skill existed to escape "Claude's 200K limit," a premise Opus 5 retired. The `features/local-llm` page is removed. Added in v7.3.1: **Claude Opus 5 adopted end-to-end**, plus two real bug fixes. The default model pin is now `claude-opus-5` (dateless, no `[1m]` suffix — Opus 5 is 1M context by default *and* max). Effort posture follows Anthropic's Opus 5 guidance: default `high`, escalate to `xhigh` for demanding agentic work, `max` only when justified — replacing the old always-on `xhigh` floor. Delegation gained a size/independence threshold (Opus 5 over-spawns without caps) and the Fable Method's verifier-≠-worker gate is now scoped to substantive work, ranking deterministic checks above a spawned verifier. **Fixed:** `/ccc-doctor` had been failing on correctly-configured repos since v6.0 (it demanded an Opus pin for four agents promoted to Fable 5 in v6.0; a synthetic test fixture drifted with the wrong expectation and hid it — the doctor is now asserted against the real repo), and complexity scores ≥93 emitted an invalid `--effort ultra`, so the *hardest* tasks were the ones failing at dispatch. `skills/advisor` also documented an impossible model pair once the executor moved to Opus 5. Added in v7.3.0: Commander finally notices its own updates — a SessionStart hook (`hooks/update-nudge.js`, 24h-cached, offline-silent) compares the installed version against GitHub `main` and, when newer, spells out the exact fix — marketplace update → plugin update → restart. **`/ccc-update`** (81st plugin skill) is the click-first version of that same walkthrough, and `/ccc-doctor` now diagnoses every stale-install layer (marketplace clone behind `main`, old `cache/<version>` dirs, stale Desktop inline copies, catalog-cache age) and offers — with consent — to enable marketplace auto-update. The Cockpit gained an 8th tab, **Prompts** — a searchable browser across four sources (Claude Code docs library, the CCC library, the repo's prompt templates, a ReadyIQ showcase) with Copy and "Enhance this" actions. Every deck now shows a **data-freshness** stamp ("Data through <timestamp>") and warns when telemetry is stale (>24h). The suggestion engine surfaces update-available and stale-telemetry signals into Mission Control's previously-empty Suggestions panel, keyed per-project so state no longer leaks between repos. **Also fixed:** the Codex mirror was silently dead on every install — it rewrote hooks to a variable Codex never sets — and now carries the correct 10 supported hook events (was 6), strips async handlers Codex would otherwise skip, and documents Codex's hook trust gate. Added in v7.2.0: a shared **Commander decks** switcher across every artifact (so users can see and jump between all of them) plus two new decks — **Usage & Cost** (`/ccc-usage`: burn, savings, cost-by-app) and **Safety** (`/ccc-safety`: blocked/auto-fixed actions + tool-failure hotspots). Added in v7.1.0: an ecosystem **Tools** tab in the Cockpit — open Commander surfaces, install the best MIT-licensed companion apps (Nimbalyst, Usage Monitor, Sniffly…), or generate an artifact dashboard. A full redesign of the Commander Cockpit shipped in v7.0.0 on the real commanderplugin.com coral brand: a filterable skill browser (rows/tiles toggle + real-time filter, replacing the one-skill-domain wall), an 11-strategy multi-select prompt enhancer with a GO button, real per-agent token/cost telemetry (recovered from the subagent transcript, deduped by `message.id`, honest "—" when unavailable), and an honest 14-domain taxonomy (no "core"). **Commander Mission Control**'s Claude+Codex rosters gained a Charts strip (cost/day, agents dispatched/day, tasks completed/week, tool failures/day — zero-dep inline SVG, theme-safe, rendered on both the live dashboard and the snapshot artifact from one shared `charts.js`), a claude-mem **History** panel (opt-in, hidden when claude-mem isn't installed), and a "derived" marker on roster rows synthesized from delegation events (Codex work that has no real start record — dimmed gauges + tooltip, never claims token/cost data it doesn't have) — shipped in v6.8.3. The Commander Cockpit — every tool clickable inside one self-contained artifact: skill/agent browser with job-first search, catalog-grounded idea generator, live prompt enhancer, your own agent analytics (Claude + Codex side by side), an agent-profile manager with edit→export, and a skill-runs logger (42nd handler) so "most-used skill" is real data — shipped in v6.8.2. Claude *and* Codex side-by-side session tracking (`source_app` tagging, Source filter + pills, `sourceApp:name` agent keys) plus the Suggestions panel were added in v6.8.1. Mission Control itself — a visual harness inside Claude Code Desktop (live zero-dep dashboard + never-blocking event feed hook + CSP-safe snapshot artifact) plus a 5-signal proactivity wave — shipped in v6.8.0. Recent additions: /ccc-prompt-fix, claude-code-library, /ccc-spawn, /ccc-broadcast — see CHANGELOG for per-version detail. · npm package `cc-commander@7.4.2` (version tracked in `package.json`)
...
**v7.4.2:** 83 plugin skills (13 /ccc-* specialist workflows + 11 ccc-* domain routers + 7 channel/CI/ECC/setup skills + diagnostic/meta + vendor-sourced + lifecycle + /ccc-deploy + /ccc-rollback + /ccc-onboard + /ccc-fleet-viz + /ccc-changelog + /ccc-doctor + /ccc-upgrade + /ccc-tuneup + /ccc-orchestrate + /ccc-handoff + /ccc-adopt + /ccc-relay + /ccc-smb-ops + /ccc-broadcast + /ccc-spawn + /ccc-prompt-fix + /ccc-mission-control + /ccc-usage + /ccc-safety + /ccc-update + /ccc-console + /ccc-adhd) + 22 specialist agents + 23 lifecycle hooks × 44 handlers (incl. PermissionRequest + SessionStart orchestrator + 5 ECC hooks ported) + 2 credential-free bundled MCP servers (context7 + sequential-thinking) + 16 opt-in via /ccc-connect — **core free forever**, sustained by transparent affiliate links in /ccc-connect + Kevin's consulting practice. Hosted MCP is **live** at mcp.commanderplugin.com, free for everyone with a 100-call/mo anti-abuse cap; hosted-infrastructure Pro tier planned later — all content stays free forever.
...
- `commander/cowork-plugin/` — Desktop plugin (83 skills, 22 sub-agent personas, 23 hooks × 44 handlers, 2 credential-free bundled MCP servers per `.mcp.json` + 16 opt-in via `/ccc-connect`)
...
│   ├── skills/                  # 83 plugin skills (23 ccc-* workflows + 7 channel/CI/ECC/setup + remaining lifecycle + 2 vendor-sourced + 25 other + /ccc-tuneup + /ccc-update + /ccc-console)
...
```
━━ CCC7.3.0│🔥Opus1M│🔑gAA│🧠▐██45%░░▌│⏱️▐██░░▌6%│📅▐██░░▌34%│💰$2.34│⬆️640K⬇️694K│⏰8h0m│🎯467│📋CC-150│📂~/project
```
...
**Version:** 7.4.2 (Desktop-first plugin, Claude Agent SDK sub-agent architecture — see `package.json`)

**Stats:** 467 skills total, 83 plugin skills, 22 sub-agent personas, 23 lifecycle hooks (44 handlers), 2 bundled MCP servers + 16 opt-in, 83 commands, 24 JS hooks, 19 vendors, 10 themes
...
CCC ships 83 focused `/ccc-*` plugin skills as the curated front door, then routes into 467 CCC ecosystem skills across 11 domains. It is a meta-layer over the broader Claude skill ecosystem — not a replacement for it.
...
|--------|---------|------------|
| Marketplace (recommended) | `/plugin install commander` | Full plugin with all 83 skills + MCPs + hooks |
...
- 83 `/ccc-*` plugin skills are the curated front door; they route into 467+ deeper skills
- `CONNECTORS.md` defines 13 tool-agnostic connector categories via `~~category` placeholders — skills are tool-agnostic
- Everything core free forever: 83 plugin skills, 22 agents, 2 credential-free bundled MCP servers (context7 + sequential-thinking) + 16 opt-in via /ccc-connect, 23 lifecycle hooks × 44 handlers. No feature gating, no paywalls, no license checks.
