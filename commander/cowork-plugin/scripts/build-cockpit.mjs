#!/usr/bin/env node
/**
 * build-cockpit.mjs (v7.3.0, W2+/codex 7)
 *
 * Canonical location: this file lives INSIDE the plugin (commander/
 * cowork-plugin/scripts/) so a marketplace-only install (which ships
 * agents/hooks/lib/menus/rules/skills/scripts — see .claude-plugin/
 * plugin.json — but NOT the repo root's `commander/contract.json` or the
 * top-level `skills/` ecosystem catalog) can still run it to regenerate
 * the Cockpit. `../../../scripts/build-cockpit.mjs` (repo root) is a thin
 * spawn shim kept for repo devs' muscle memory — see that file.
 *
 * Every path below resolves relative to THIS file's own location
 * (import.meta.url), not `process.cwd()`, so it behaves the same whether
 * invoked from the repo root, from inside a plugin cache directory, or
 * via the repo-root shim. Two sources only exist in a full dev checkout,
 * one directory (commander/contract.json) and two directories up
 * (top-level skills/) from this file — resolveContract() and
 * resolveEcosystemSkillsDir() both degrade gracefully (null) rather than
 * throwing when they're absent, and the rest of this file treats "no
 * ecosystem catalog" as an honest, smaller Cockpit (plugin skills +
 * agents only), never a crash. See buildDocument()'s doc comment.
 */
...
import { readTopSkills } from '../lib/top-skills.js';
import { brandBaseCss } from '../lib/brand-css.js';
import { deckStripHtml, deckStripCss } from '../lib/deck-switcher.js';

const HERE = path.dirname(fileURLToPath(import.meta.url));
const PLUGIN_ROOT = path.resolve(HERE, '..'); // commander/cowork-plugin — always present (this file ships inside it)
...
const STALE_THRESHOLD_MS = 24 * 60 * 60 * 1000; // v7.3.0 staleness — matches the other 3 decks
...
function buildTierMap(ecosystemSkillsDir) {
  const tiersPath = path.join(ecosystemSkillsDir, '_tiers.json');
  const config = JSON.parse(fs.readFileSync(tiersPath, 'utf8'));
...
function buildEcosystemSkills(files, tierById, ecosystemSkillsDir) {
  return files.map((filePath) => {
    const relativePath = path.relative(ecosystemSkillsDir, filePath);
...
// A plugin-only build (no ecosystem catalog present — see resolveEcosystemSkillsDir)
// can't resolve every idea's referenced skill, since some ideas point at
// ecosystem-only skills. Rather than crash the whole Cockpit build (the old
// findCatalogSkill() threw on a miss), skip the individual idea and keep going —
// an honestly SHORTER ideas list beats no Cockpit at all. In a full dev checkout
// (ecosystem catalog present) every idea still resolves, so this never changes
// the full-repo build's output — see cockpit.test.js's ideas-length pin.
...
// Repo-root-relative sources (v7.3.0, W2+/codex 7): contract.json and the
// top-level ecosystem skills/ catalog live OUTSIDE commander/cowork-plugin/,
// so they only exist in a full dev checkout of the repo — never in a
// marketplace-only plugin install. Both degrade to null/empty rather than
// throwing; buildDocument() below treats their absence as an honest,
// smaller Cockpit (plugin skills + agents only), not a crash.
...
function resolveEcosystemSkillsDir() {
  try {
    return fs.statSync(ECOSYSTEM_SKILLS_DIR_CANDIDATE).isDirectory() ? ECOSYSTEM_SKILLS_DIR_CANDIDATE : null;
  } catch {
    return null;
  }
}

// Falls back to the plugin's own manifest version when contract.json is
// unavailable (plugin-only install) — still never fabricated: it's the
// same version string ${CLAUDE_PLUGIN_ROOT}/.claude-plugin/plugin.json
// carries, which every other skill in this plugin already reads live.
...
// v7.3.0, Item 6 — telemetry freshness. Newest ts across the SAME four local
// logs buildAnalytics() reads (agent-runs/subagent-runs/tasks/events),
// computed independently of whether buildAnalytics() itself found enough
// signal to emit a non-empty analytics payload — a fully-empty analytics
// object can still carry a "no telemetry / check your hooks" note. Read once
// here rather than threading buildAnalytics()'s internal `sources` out
// through its signature: small, self-contained, matches the read-it-again
// convention already used across this plugin's lib/ tree.
function computeTelemetryFreshness(baseDir, nowMs) {
  const files = [
  ...
  for (const file of files) {
    for (const entry of readJsonl(file)) {
      ...
      const ms = parseTimestamp(entry.ts);
      if (ms !== null && (dataThroughMs === null || ms > dataThroughMs)) dataThroughMs = ms;
    }
  }
  const stale = hasAnySourceRow && (dataThroughMs === null || nowMs - dataThroughMs > STALE_THRESHOLD_MS);
  return { dataThroughMs, hasAnySourceRow, stale };
}
...
// v7.3.0, W2+/codex 7 — plugin-only degrade: contract.json + the top-level
// ecosystem skills/ catalog only exist in a full dev checkout (both live
// outside commander/cowork-plugin/, which is all a marketplace install
// ships). When either is absent, the Cockpit still builds — honestly
// smaller (plugin skills + agents only, no contract-count assertion, no
// ideas that could only resolve against an ecosystem-only skill) — rather
// than crashing. `meta.pluginOnly` marks which mode produced the payload.
async function buildDocument() {
  const contract = resolveContract();
  const ecosystemSkillsDir = resolveEcosystemSkillsDir();
  const pluginOnly = !contract || !ecosystemSkillsDir;

  const pluginFiles = walkFilesNamed(PLUGIN_SKILLS_DIR, 'SKILL.md');
  const ecosystemFiles = ecosystemSkillsDir ? walkFilesNamed(ecosystemSkillsDir, 'SKILL.md') : [];
  const agentCount = fs
    .readdirSync(AGENTS_DIR, { withFileTypes: true })
  ...
  // Only assert against contract.json when we actually have one to check
  // against — a plugin-only install has no local ground truth to compare to.
  if (contract) assertContractCounts(contract, counts);

  const pluginSkills = buildPluginSkills(pluginFiles);
  const ecosystemSkills = ecosystemSkillsDir
    ? buildEcosystemSkills(ecosystemFiles, buildTierMap(ecosystemSkillsDir), ecosystemSkillsDir)
    : [];
  ...
  const baseDir = path.join(home, '.claude', 'commander');
  const nowMs = Date.now();
  const freshness = computeTelemetryFreshness(baseDir, nowMs);

  const payload = {
    meta: {
      version: contract ? contract.version : readPluginVersion(),
      generatedAt: new Date(nowMs).toISOString(),
      pluginSkills: counts.pluginSkills,
      ecosystemSkills: counts.ecosystemSkills,
      agents: counts.agents,
      pluginOnly,
      dataThroughMs: freshness.dataThroughMs,
      hasAnySourceRow: freshness.hasAnySourceRow,
      telemetryStale: freshness.stale,
    },
    skills,
    agents,
    jobs: JOBS,
