...
- `commander/cowork-plugin/` — Desktop plugin (82 skills, 22 sub-agent personas, 23 hooks × 43 handlers, 2 credential-free bundled MCP servers per `.mcp.json` + 16 opt-in via `/ccc-connect`)
...
│   ├── skills/                  # 82 plugin skills (23 ccc-* workflows + 7 channel/CI/ECC/setup + remaining lifecycle + 2 vendor-sourced + 25 other + /ccc-tuneup + /ccc-update)
...
**Stats:** 467 skills total, 82 plugin skills, 22 sub-agent personas, 23 lifecycle hooks (43 handlers), 2 bundled MCP servers + 16 opt-in, 83 commands, 24 JS hooks, 19 vendors, 10 themes
...
- Everything core free forever: 82 plugin skills, 22 agents, 2 credential-free bundled MCP servers (context7 + sequential-thinking) + 16 opt-in via /ccc-connect, 23 lifecycle hooks × 43 handlers. No feature gating, no paywalls, no license checks.
