...
**Current Release:** v6.7.3 — adds `/ccc-prompt-fix` (the auto prompt fixer: diagnoses your prompt against the 6 prompt-quality patterns, returns a sharper rewrite with the reasoning, and suggests the right library prompt for your task) + integrates the `claude-code-library` (52 Claude Code prompts by SDLC phase + a 7-module best-practices course, adapted from Anthropic docs). Prior: v6.7.2 /ccc-spawn · v6.7.1 /ccc-broadcast · v6.7.0 /ccc-relay + ccc-smb-ops. · npm package `cc-commander@6.7.3` (version tracked in `package.json`)
