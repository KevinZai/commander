...
> Updated: 2026-07-17 | Version: 6.8.3 (see package.json) | Non-coder friendly. Practical examples throughout.
...
- [The 78 Plugin Skills](#the-77-plugin-skills) — The curated plugin surface
...
### The Appendices
- [CC Commander](#cc-commander) *(v6.8.3 — Desktop plugin + CLI, Desktop-first)*
...
> **Distilled from Claude Fable 5, written to run on any model.** Frontier-model results come from the *system around the model*, not just raw capability. Every instinct that makes Fable reliable — refusing a single pass, isolating blast radius, verifying against ground truth, treating context as disposable, operating like a PM instead of a typist — can be externalized as a gate any model can follow. Arm it with `/ccc-fable on`. Full text: `commander/cowork-plugin/rules/fable-method.md`.

**The one-line version:** *judgment as process.* When the model is weaker, the process must be stronger.

**The 12 pillars, as gates (checkable), not values (aspirational):**

| # | Pillar | Gate question |
|---|--------|----------------|
...
| 10 | The proactive PM posture | Did I evaluate IMPROVE/SCOPE/AUDIT, and did I resist executing the flags inline? |
...
./install.sh --skills=full        # All 467+ skills (original behavior)
```
...
### 🛠️ Plugin Workflows (v6.8.3)

CC Commander is now a Claude Code plugin. The primary UX is plain `/ccc-*` slash commands with a native AskUserQuestion chip picker. The plugin ships 78 skills; these core workflow shortcuts require no menu traversal:
...
> *v6.8.3* — CC Commander's sub-agent architecture is built on the brain/hands pattern described in Anthropic's Claude Agent SDK.
...
> *v6.8.3* — **Primary surface: Claude Code Desktop (aka Cowork Desktop).** 78 plugin skills, 22 specialist sub-agents, 2 bundled MCPs (16 opt-in), 23 lifecycle hooks (42 handlers). Click-first via AskUserQuestion. A CLI also exists for power users. Install via Settings → Plugin Marketplace → Add from GitHub (`KevinZai/commander`).
>
...
Claude Code session
  ...
  |
  +-- /ccc-build, /ccc-review, ...    (78 plugin skills)
...
| **Skill browser** | Browse all 467+ skills from within Commander |
...
| **Desktop-first** | 78 plugin skills, 22 agents, 2 bundled MCPs (16 opt-in), 23 lifecycle hooks (42 handlers) — install via Settings → Plugin Marketplace in Claude Code Desktop / Cowork Desktop |
...
> *Appendix: v6.8.3 — How CCC thinks before it acts.*
...
`recommendSkills(task, techStack)` ranks all 467+ skills using three signals:
