...
- `commander/cowork-plugin/` — Desktop plugin (63 skills, 22 sub-agent personas, 23 hooks × 38 handlers, 2 credential-free bundled MCP servers per `.mcp.json` + 16 opt-in via `/ccc-connect`)
...
│   ├── skills/                  # 63 plugin skills (23 ccc-* workflows + 6 channel/CI/ECC + remaining lifecycle + 2 vendor-sourced + 25 other + /ccc-tuneup)
...
**Version:** 6.0.0 (Desktop-first plugin, Claude Agent SDK sub-agent architecture — see `package.json`)

**Stats:** 457 skills total, 63 plugin skills, 22 sub-agent personas, 23 lifecycle hooks (38 handlers), 2 bundled MCP servers + 16 opt-in, 83 commands, 28 JS hooks, 18 vendors, 10 themes
...
CCC ships 63 focused `/ccc-*` plugin skills as the curated front door, then routes into 457 CCC ecosystem skills across 11 domains. It is a meta-layer over the broader Claude skill ecosystem — not a replacement for it.
...
|--------|---------|------------|
| Marketplace (recommended) | `/plugin install commander` | Full plugin with all 63 skills + MCPs + hooks |
...
- 63 `/ccc-*` plugin skills are the curated front door; they route into 457+ deeper skills
- `CONNECTORS.md` defines 13 tool-agnostic connector categories via `~~category` placeholders — skills are tool-agnostic
- Everything free for now: 63 plugin skills, 22 agents, 2 credential-free bundled MCP servers (context7 + sequential-thinking) + 16 opt-in via /ccc-connect, 23 lifecycle hooks × 38 handlers. No feature gating, no paywalls, no license checks.
