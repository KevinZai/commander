...
CC Commander's primary product as of v5.1.2. Claude Cowork Desktop / Claude Code Desktop install:
...
**v4.1 (next):** hosted MCP (free with 100-call/mo anti-abuse cap, no paid upgrade) + Commander Hub marketplace + Anthropic Connectors directory submission.

Plugin directory: `commander/cowork-plugin/` — CONNECTORS.md documents all 10 connector categories.

### Architecture note (v5.1.2+)
...
- `commander/cowork-plugin/` — Desktop plugin (62 skills, 22 sub-agent personas, 23 hooks × 38 handlers, 2 credential-free bundled MCP servers per `.mcp.json` + 16 opt-in via `/ccc-connect`)
...
│   ├── skills/                  # 62 plugin skills (23 ccc-* workflows + 6 channel/CI/ECC + remaining lifecycle + 2 vendor-sourced + 25 other + /ccc-tuneup)
...
├── skills/              # 457 skills organized by category
...
18 vendor submodules in `vendor/`. Smart orchestrator scores tools: capability 50% + stars 15% + recency 15% + user pref 20%.
...
**Note:** `claude-mem` was previously vendored but was removed in v5.1.2 — it ships under AGPL-3.0 which is incompatible with CC Commander's MIT license. Users can install `claude-mem` separately via `npm install claude-mem` or `/ccc-connect`; it is treated as an external opt-in MCP, not a bundled vendor.

## Footer Bar
CCC renders a rich status footer on every menu screen and recommends it for `/ccc` mid-session:

```
━━ CCC4.0.0│🔥Opus1M│🔑gAA│🧠▐██45%░░▌│⏱️▐██░░▌6%│📅▐██░░▌34%│💰$2.34│⬆️640K⬇️694K│⏰8h0m│🎯457│📋CC-150│📂~/project
```
...
| `/ccc skills` | Skill browser | 457 skills by category, preview, try |
...
**Version:** 5.1.2 (Desktop-first plugin, Claude Agent SDK sub-agent architecture — see `package.json`)

**Stats:** 457 skills total, 62 plugin skills, 22 sub-agent personas, 23 lifecycle hooks (38 handlers), 2 bundled MCP servers + 16 opt-in, 83 commands, 28 JS hooks, 18 vendors, 10 themes
...
- Free for now: all skills, all agents, all hooks, all MCP servers — sustained by affiliate links + Kevin's consulting
...
CCC ships 62 focused `/ccc-*` plugin skills as the curated front door, then routes into 457 CCC ecosystem skills across 11 domains. It is a meta-layer over the broader Claude skill ecosystem — not a replacement for it.
...
|--------|---------|------------|
| Marketplace (recommended) | `/plugin install commander` | Full plugin with all 62 skills + MCPs + hooks |
...
- 62 `/ccc-*` plugin skills are the curated front door; they route into 457+ deeper skills
- `CONNECTORS.md` defines 13 tool-agnostic connector categories via `~~category` placeholders — skills are tool-agnostic
- Everything free for now: 62 plugin skills, 22 agents, 2 credential-free bundled MCP servers (context7 + sequential-thinking) + 16 opt-in via /ccc-connect, 23 lifecycle hooks × 38 handlers. No feature gating, no paywalls, no license checks.
