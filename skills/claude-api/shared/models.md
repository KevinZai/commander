...
| Claude Opus 5      | `claude-opus-5`      | `claude-opus-5`               | 1M      | 128K       | $5 / $25                  | Active — recommended default |
...
- **Claude Fable 5** — Anthropic's most capable widely released model (GA 2026-06-09). No dateless alias — use the pinned ID `claude-fable-5` directly. Adaptive thinking is **always-on**; does NOT support the `extended-thinking` parameter. Uses the Opus-4.7+ tokenizer: same text produces ~30% more tokens than pre-4.7 models. Available: Claude API, Bedrock (`anthropic.claude-fable-5`), Vertex (`claude-fable-5`), Microsoft Foundry (200K ctx on Foundry). Knowledge cutoff Jan 2026. Pick Fable 5 when you need maximum capability; note it is 2× the price of Opus 5.
- **Claude Opus 5** — Current flagship; best cost/capability ratio for production agents and coding, and the recommended default model. Dateless model ID (no date suffix). Adaptive thinking always-on (no `extended-thinking` param); effort defaults to `high` on all surfaces. Adds Fast mode (2.5× speed, ~2× cost) and `xhigh`/`max` effort levels. 1M context window. 128K max output (requires streaming for large outputs). Same pricing as its predecessor, Opus 4.8 ($5/$25 per MTok).
...
|-------------------|---------------------|-------------------------------|--------|
| Claude Opus 4.8   | `claude-opus-4-8`   | —                             | Active — still valid, no longer the default recommendation |
| Claude Opus 4.7   | `claude-opus-4-7`   | —                             | Active |
| Claude Opus 4.6   | `claude-opus-4-6`   | —                             | Active (prefer 5) |
| Claude Opus 4.5   | `claude-opus-4-5`   | `claude-opus-4-5-20251101`    | Active |
| Claude Sonnet 4.5 | `claude-sonnet-4-5` | `claude-sonnet-4-5-20250929`  | Active |

> **Opus 4.8** — $5/$25, 1M ctx / 128K out, adaptive thinking. Still fully supported; use it if the user explicitly asks for it, otherwise prefer `claude-opus-5`.
