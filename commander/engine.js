...
class KitCommander {
  ...
  async executeBuild(task) {
    ...
    var sysPrompt = (function() {
      ...
      var prompt = "You are the PLANNER (Opus). Your job: plan the work, delegate implementation to Sonnet workers, then audit the results. WORKFLOW: 1) Plan — break the task into steps. 2) Delegate — spawn Sonnet subagents (model: sonnet) for each implementation step. Use the Agent tool with model='sonnet'. 3) Audit — review all subagent outputs yourself (Opus). Verify correctness, run tests, check for issues. Only mark done after YOUR audit passes. CONTEXT MANAGEMENT: Use subagents for ALL file writes, code generation, and research. Keep YOUR context for planning and auditing only. Read files in small chunks (offset/limit for >500 lines). If a task has 3+ independent parts, parallelize subagents." + (pluginInstructions ? " Use these tools in sequence: " + pluginInstructions + "." : "");
